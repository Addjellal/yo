...
| `calcul-parallele` | Parallel Computing Toolbox — exécution parallèle. | 9 |
| `cartographie` | Mapping Toolbox — géodésie et cartographie. | 4 |
| `coder` | MATLAB Coder — génération de code C. | 2 |
| `communications` | Communications Toolbox — transmissions numériques. | 16 |
...
| `dsp` | DSP System Toolbox — traitement du signal en temps réel. | 8 |
...
| `flou` | Fuzzy Logic Toolbox — logique floue. | 12 |
...
| `images` | Image Processing Toolbox — traitement d'images. | 27 |
...
| `matlab` | MATLAB de base — fonctions écrites dans le langage lui-même. | 14 |
| `mpc` | Model Predictive Control Toolbox — commande prédictive. | 3 |
| `navigation` | Navigation Toolbox — localisation et planification. | 5 |
| `ondelettes` | Wavelet Toolbox — analyse en ondelettes. | 9 |
| `optimisation` | Optimization Toolbox — optimisation sous contraintes. | 7 |
| `optimisation-globale` | Global Optimization Toolbox — optimisation globale. | 4 |
...
| `signal` | Signal Processing Toolbox — traitement du signal. | 17 |
...
| `statistiques` | Statistics and Machine Learning Toolbox — statistiques et apprentissage. | 27 |
| `symbolique` | Symbolic Math Toolbox — calcul formel. | 14 |
| `vehicule` | Vehicle Dynamics / Powertrain — dynamique du véhicule. | 4 |
| `vision` | Computer Vision Toolbox — vision par ordinateur. | 10 |

Les fonctions natives — 561, écrites en C++ — couvrent le MATLAB de base :
