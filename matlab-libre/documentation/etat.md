...
| simulink | 27 | schémas-blocs : soixante-seize types de blocs, signaux vecteurs, boucles algébriques, pas fixe et pas variable avec passages par zéro, linéarisation, équilibre |
...
| Simulink | soixante-seize types de blocs — sources, opérations, logique, non-linéarités, aiguillage, continus, discrets, sous-systèmes, sorties — sur des signaux scalaires, vecteurs et matrices, avec des blocs à plusieurs sorties ; chaque modèle est compilé comme dans Simulink — paramètres, ports, dimensions, périodes vérifiés, chaque erreur nommant le bloc par son chemin — puis ordonné ; les boucles algébriques sont résolues par Newton ; neuf solveurs : ode1 à ode5 et FixedStepDiscrete à pas fixe, ode45, ode23 et ode23s (raide) à pas variable, avec détection et localisation des passages par zéro, arrêt exact sur les instants d'échantillonnage et les cassures des sources ; les sous-systèmes conditionnels — Enable, Trigger, If, Switch Case, Merge —, avec la tenue ou la remise à zéro de leurs sorties et de leurs états ; les blocs de code — Fcn, MATLAB Function avec ses variables persistantes, Interpreted MATLAB Function, S-fonctions de niveau 1 — et le bloc Chart, qui fait tourner une machine Stateflow un pas par instant ; la boîte « Paramètres de configuration » (Ctrl+E) règle le solveur, les tolérances et les diagnostics ; `linmod`, `dlinmod` et `trim` ; `save_system` écrit un `.m` qui rebâtit le modèle, et un second chemin un programme qui *fait ce qu'il fait* ; l'éditeur du bureau pose, câble, règle et simule, sous-systèmes compris | ode15s, ode23t, ode113 ; sous-systèmes itérés et appelés par fonction ; bus et types de données ; masques et bibliothèques ; S-fonctions de niveau 2 ; états hiérarchiques de Stateflow ; lecture des `.slx` |
