...
```
PRINT  Enregistre une figure.
    PRINT(NOM) enregistre la figure courante sous le nom donné.
    PRINT(NOM,PILOTE) impose le pilote ; seul '-dsvg' est géré.

    Comme SAVEAS, PRINT n'écrit que du SVG : demander '-dpng' ou '-dpdf'
    lève une erreur. Un nom sans extension reçoit '.svg'.
...
SAVEAS  Enregistre une figure dans un fichier.
    ...
    SAVEAS(F,NOM,FORMAT) impose le format.

    MatLibre dessine en SVG de bout en bout : le tracé est vectoriel,
    sans matriceur. Le seul format écrit est donc 'svg', et demander un
    autre format lève une erreur plutôt que de déposer le fichier
    ailleurs en silence. Un nom sans extension reçoit '.svg'.
