...
function [lignes, compteur] = batir(modele, cible, lignes, compteur)
    ...
    for l = 1:size(liens, 1)
        ...
        % le fichier reste celui qu'on aurait tapé.
        if liens(l, 4) > 1
            lignes{end+1} = sprintf('    %s = add_line(%s, %s, %s, %d, %d);', ...
                                    cible, cible, ...
                                    citer(modele.blocs{liens(l, 1)}.nom), ...
                                    citer(modele.blocs{liens(l, 2)}.nom), ...
                                    liens(l, 3), liens(l, 4));   %#ok<AGROW>
        else
