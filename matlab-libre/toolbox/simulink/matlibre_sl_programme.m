...
%   Voir aussi SAVE_SYSTEM, LOAD_SYSTEM, SIM, MATLIBRE_SL_COMPILER.
    ...
    % Un intégrateur qui se remet, lit sa condition initiale d'une entrée,
    % ou montre ses ports de saturation et d'état : pas encore écrit.
    for k = find(strcmp(c.types, 'integrator'))
        w = (numel(c.seg{k}) - 6) / 2;
        if any(c.seg{k}(2 + 2 * w:3 + 2 * w) ~= 0) || c.nOut(k) > 1
            error('Simulink:programme:BlocNonEcrit', ...
                  ['L''integrateur ''%s'' a une remise externe, une condition initiale ' ...
                   'externe, ou un port de saturation ou d''etat : le programme engendre ' ...
                   'ne les ecrit pas encore.'], c.chemins{k});
        end
    end
