...
const BlocBibliotheque blocs[] = {
    ...
    {"Sources", "chirp", "Une sinusoïde dont la fréquence croît", "'f1', 0.1, 'T', 10, 'f2', 1"},
    {"Sources", "signalgenerator", "Sinus, carré, dent de scie ou bruit",
     "'WaveForm', 'square', 'Units', 'Hertz'"},
    {"Sources", "bandlimitedwhitenoise", "Un bruit blanc de puissance donnée",
     "'Cov', 0.1, 'Ts', 0.01"},
    {"Sources", "counterlimited", "Compte jusqu'à sa limite, puis repart de zéro",
     "'uplimit', 7"},
    {"Sources", "counterfreerunning", "Compte sans fin, modulo 2^NumBits", "'NumBits', 8"},
    {"Sources", "repeatingsequencestair", "Une suite de valeurs, tour à tour",
     "'OutValues', [3 1 4 2 1]"},
    ...
    {"Logique", "intervaltest", "Vaut 1 quand l'entrée est dans l'intervalle",
     "'uplimit', 0.5, 'lowlimit', -0.5"},
    ...
    {"Aiguillage", "manualswitch", "L'une ou l'autre entrée, au choix", "'sw', '1'"},
    {"Aiguillage", "datastorememory", "Une mémoire partagée, désignée par son nom",
     "'DataStoreName', 'A', 'InitialValue', 0"},
    {"Aiguillage", "datastoreread", "Lit la mémoire partagée de son nom", "'DataStoreName', 'A'"},
    {"Aiguillage", "datastorewrite", "Écrit dans la mémoire partagée de son nom",
     "'DataStoreName', 'A'"},
    {"Aiguillage", "ratetransition", "Passe d'une période d'échantillonnage à une autre",
     "'OutPortSampleTime', 0.1"},
    {"Aiguillage", "ic", "Sa valeur au premier instant, puis son entrée", "'Value', 0"},
    {"Aiguillage", "width", "Le nombre d'éléments de son entrée", ""},
    ...
    {"Continu", "secondorderintegrator", "Intègre deux fois : x et dx/dt",
     "'ICX', 0, 'ICDXDT', 0"},
    ...
    {"Discret", "discretederivative", "K (u - u d'avant) / Ts", "'gainval', 1"},
    {"Discret", "difference", "u - u d'avant", ""},
    {"Discret", "tappeddelay", "Les N dernières valeurs, en un vecteur",
     "'NumDelays', 4, 'samptime', 0.1"},
    {"Discret", "discretezeropole", "Une transmittance en z par ses zéros et pôles",
     "'Zeros', [], 'Poles', 0.5, 'Gain', 1, 'SampleTime', 0.1"},
    ...
    {"Sous-systèmes", "If Action Subsystem", "Calcule quand un If ou un Switch Case le désigne",
     ""},
    {"Sous-systèmes", "Function-Call Subsystem",
     "Calcule quand un Function-Call Generator l'appelle", ""},
    {"Sous-systèmes", "For Iterator Subsystem", "Calcule N fois par pas", ""},
    {"Sous-systèmes", "While Iterator Subsystem", "Calcule tant qu'une condition est vraie",
     ""},
    {"Sous-systèmes", "functioncallgenerator",
     "Appelle le sous-système relié, à chaque instant de sa période", "'sample_time', 0.1"},
    {"Sous-systèmes", "foriterator", "Posé dans un sous-système, l'y fait calculer N fois",
     "'IterationLimit', 5"},
    {"Sous-systèmes", "whileiterator",
     "Posé dans un sous-système, l'y fait calculer tant que cond est vraie",
    ...
    {"Sorties", "xygraph", "Relève y en fonction de x", ""},
    {"Sorties", "tofile", "Écrit le temps et le signal dans un fichier MAT",
     "'Filename', fullfile(tempdir, 'matlibre_tofile.mat'), 'MatrixName', 'donnees'"},
